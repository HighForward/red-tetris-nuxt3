    ...
    <div class="flex flex-col">
      <div class="flex justify-around px-8">
          ...
          <div v-if="othersBoards.length > 0" class="others__container">
              <div v-for="board in othersBoards" class="p-4">
                  ...
                  <div class="flex flex-col w-full p-4 justify-center items-center">
                      <span>Partie de:<span class="text-content pl-2 font-bold">{{ " " + board?.player?.username }}</span></span>
                      <span>Score:<span class="text-content pl-2 font-bold">{{ " " + board?.score }}</span></span>
                  </div>
          ...
          <div v-if="boards[board_index]" class="flex flex-col items-center h-full font-semibold">
              <div class="flex flex-col w-full p-4 justify-center items-center">
                  <span>Partie de:<span class="text-content pl-2 font-bold">{{ " " + boards[board_index]?.player?.username }}</span></span>
                  <span>Score:<span class="text-content pl-2 font-bold">{{ " " + boards[board_index]?.score }}</span></span>
              </div>
