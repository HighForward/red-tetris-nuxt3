...
const player_board = game_boards.value.find(b => b.player.id === player.value.id)
  ...
  <div class="flex justify-around px-8">
    ...
    <div class="flex flex-col items-center h-full font-semibold">
      ...
      <div class="flex flex-col justify-center bg-gray rounded-sm" style="box-shadow: 0px 0px 25px 5px rgba(0, 0, 0, 0.35);">
        <div class="flex flex-row" v-for="(row, y) in player_board?.board" :key="y">
          <div class="h-8 w-8" v-for="(item, x) in row" :key="x">
            <div class="w-full h-full" :class="setColor(item)" style="box-shadow: inset 0px 0px 0px 1px rgba(0, 0, 0, 0.5);" ref="blockRef"></div>
          </div>
        </div>
      </div>
